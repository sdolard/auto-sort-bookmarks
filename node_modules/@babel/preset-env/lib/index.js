import { isLess, coerce, parse, normalize } from 'verkit';
...
function removeUnsupportedItems(items, babelVersion) {
  items.forEach(item => {
    if (Object.hasOwn(minVersions, item) && isLess(babelVersion, minVersions[item])) {
      items.delete(item);
    }
  });
}
...
function normalizeOptions(opts) {
  ...
  if (opts.useBuiltIns) {
    throw new Error("The 'useBuiltIns' option has been removed. Please use babel-plugin-polyfill-corejs3 instead.");
  }
...
const index = declarePreset((api, opts) => {
  ...
  if (isLess(api.version, "7.13.0") || opts.targets || opts.configPath || opts.browserslistEnv || opts.ignoreBrowserslistConfig) {
